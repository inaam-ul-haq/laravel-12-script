<x-layouts.auth>
    <x-slot name="pageTitle">{{__('language.create_post')}}</x-slot>

    <x-auth.form form-action="{{ route('blog.post.store') }}" enctype="true" form-id="submitProduct">
        <div class="row">
            <div class="col-md-9">
                <div class="row">
                    <div class="col-md-12">
                        <x-auth.card card-header="{{__('language.create_post')}}">
                            <div class="product_title">
                                <x-auth.input-field type="text" name="title" id="title" required="true"
                                    place="{{ __('language.title') }}" val="" extraclasses=""
                                    label="{{ __('language.title') }}" />
                                <small class="text-danger">{{ $setting->url }}/<span id="editableSlug"
                                        style="border-bottom: 1px solid red; cursor: pointer;"></span></small>

                                <input type="hidden" name="slug" id="slugInputField">
                            </div>

                            <div class="mt-3">
                                <x-auth.text-editor name='short_description' id='short_description'
                                    place="{{__('language.short_description_place')}}" val="" required=""
                                    label="{{__('language.short_description_title')}}" />
                            </div>

                            <div class="mt-3">
                                <x-auth.text-editor name='content' id='content'
                                    place="{{__('language.description_title')}}" val="" required=""
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <x-auth.input-field type="text" name="focus_keyword" id="focus_keyword"
                                        required="true" place="{{ __('language.focus_keyword') }}" val=""
                                        extraclasses="" label="" />
                                </div>

                                <div class="col-md-12 mt-3">
                                    <x-auth.input-field type="text" name="meta_title" id="meta_title" required="true"
                                        place="{{ __('language.meta_title') }}" val="" extraclasses="" label="" />
                                </div>

                                <div class="col-md-12 mt-3">
                                    <x-auth.text-area type="text" name="meta_description" id="meta_description"
                                        required="true" place="{{ __('language.meta_description') }}" val=""
                                        extraclasses="" label="" />
                                </div>
                            </div>
